...
a different CLI never restores that CLI's old history. Selecting a different model
on the same client does not resume the previous model's native thread; the next
