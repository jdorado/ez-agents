...
no separate bot is required. A botless runtime may grant `--share-active` to use
standard shared runtime controls. The historical `followTelegram:true` admission
field follows that same active state even without a bot. Telegram transport is
still disabled; the flag does not enable polling, messages, or scheduling.
