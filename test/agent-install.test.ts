...
test('new agents receive independent projects, secrets and plugin bindings; duplicate names never overwrite', async () => {
  ...
  try {
    ...
    assert.equal(host.isolation,'isolated')
    ...
    assert.equal(a.EZ_ISOLATION,'isolated')
    assert.equal(a.EZ_EXECUTOR_TRANSPORT,'local')
    assert.equal(b.EZ_ISOLATION,'isolated')
    assert.equal(b.EZ_EXECUTOR_TRANSPORT,'local')
    assert.equal(JSON.parse(await readFile(join(root,'family/agent.json'),'utf8')).isolation,'isolated')
