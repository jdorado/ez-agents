...
test('macos profile blocks sibling reads and writes', async (t) => {
  ...
  await mkdir(mine); await mkdir(other); await mkdir(shared); await mkdir(additional)
  await writeFile(path.join(mine, 'own.txt'), 'own')
  await writeFile(path.join(other, 'secret.txt'), 'secret')
  await writeFile(path.join(shared, 'shared.txt'), 'shared')
  await writeFile(path.join(additional, 'additional.txt'), 'additional')
  const profile = path.join(root, 'workspace.sb')
  await writeFile(profile, macosWorkspaceProfile(workspaceSiblingDenies(mine, '/Users/ada'), [shared, additional]))
  ...
  assert.equal((await run(path.join(shared, 'shared.txt'))).code, 0)
  assert.equal((await run(path.join(additional, 'additional.txt'))).code, 0)
