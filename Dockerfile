...
RUN node --input-type=module -e 'import {readFileSync,symlinkSync} from "node:fs"; for (const [name,file] of Object.entries(JSON.parse(readFileSync("package.json","utf8")).bin)) symlinkSync(`/app/${file}`, `/usr/local/bin/${name}`);'
