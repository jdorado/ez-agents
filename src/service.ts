...
export const serviceDefinition = (directory: string, includePluginBroker = false) => {
  ...
  return { command: 'docker', args: ['compose', '--project-directory', packageRoot,
    '--env-file', path.join(root, 'docker.env'), 'up', '-d', '--wait', ...(includePluginBroker ? ['relay', 'plugin-broker'] : ['relay'])] }
}
...
export const installService = async (directory: string) => {
  ...
  const definition = serviceDefinition(directory, values.EZ_EXECUTOR_TRANSPORT === 'local')
