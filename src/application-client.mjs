...
export async function applicationCall(path, body, { url, token, fetchImpl = fetch, signal } = {}) {
  if (!token) throw new Error('Ez application credential is missing')
  if (!/^\/v1\/(?:registration|control|runs(?:\?before=r_(?:app|schedule)_[a-f0-9]{64}|\/r_(?:app|schedule)_[a-f0-9]{64}(?:\/cancel)?)?)$/.test(path)) throw new Error('Invalid Ez application operation')
  let response
  try {
