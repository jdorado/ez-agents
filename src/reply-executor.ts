...
export async function startReplyExecutor(options: ExecutorOptions) {
  ...
  try {
    ...
    const args = taskArguments(directory, broker, prompt, ['context', 'send', 'defer'])
    const preset = run.execution.preset
    if (preset.model) args.unshift('-m', preset.model)
    // Global -m precedes exec; configuration overrides are global as well.
    if (preset.effort) args.unshift('-c', `model_reasoning_effort=${JSON.stringify(preset.effort)}`)
