...
const tools = Object.entries(descriptions).map(([name, description]) => {
  ...
  return { name, description, inputSchema: {
    type: 'object', properties: name === 'context' ? {} : capability ? {input:{type:'string',maxLength:1000}} : { text: { type: 'string', maxLength: 4096 }, ...(name === 'send' ? { key: { type: 'string', pattern: '^[a-zA-Z0-9_-]{1,80}$' } } : {}) },
...
for await (const line of createInterface({ input: process.stdin })) {
  ...
  try {
    ...
    if (request.method === 'initialize') result = { protocolVersion: '2024-11-05', capabilities: { tools: {} }, serverInfo: { name: 'ez-task', version: '1' } }
      ...
      if (Object.keys(args).some(key => capabilityId ? key !== 'input' : !['text', ...(name === 'send' ? ['key'] : [])].includes(key))) throw new Error('Unexpected tool argument')
      try {
        let data: unknown
        if (capabilityId) {
          ...
          data = await runTaskCapability(toolsHome || undefined,begun.capability,args.input)
          await taskCall(controlDir,runId,'worker','capability_result',{id:capabilityId,input:args.input,lease:begun.lease})
